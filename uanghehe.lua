...
-- ==========================================
local UtilityCard = createCard("UtilityCard", 600, 5) 
                ...
                local pcallSuccess, err = pcall(function()
                    ...
                    if bp then
                        for _, child in ipairs(bp:GetChildren()) do
                            if (child:IsA("Configuration") or child:IsA("Tool")) and child:GetAttribute("HarvestedFruit") == true then
                                local id = child:GetAttribute("Id")
                                if id then
                                    if child:IsA("Configuration") then
                                        NetworkingModule.Backpack.PromoteFruit:Fire(id)
                                        task.wait(0.2)
                                    elseif child:IsA("Tool") and child.Parent == bp then
                                        local char = game.Players.LocalPlayer.Character
                            ...
                            if prompt then
                                ...
                                if isValidSeed and not ignoredSeeds[obj] then
                                    ...
                                    if checkPos and checkPos.Y < -5 then
                                        ignoredSeeds[obj] = true -- Abaikan jika jatuh ke jurang
                                    else
                                        foundSeed = obj
                                        targetPrompt = prompt
                                        break
